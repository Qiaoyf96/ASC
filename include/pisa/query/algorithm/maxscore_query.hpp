...
struct maxscore_query {
    explicit maxscore_query(topk_queue& topk, cluster_map& range_to_docid, std::vector<int>& subcluster_sizes, float mu, float eta) : m_topk(topk), m_range_to_docid(range_to_docid), m_subcluster_sizes(subcluster_sizes), m_mu(mu), m_eta(eta) {}
    ...
    void boundsum_range_query(CursorRange&& cursors_, const size_t max_clusters)
    {
        ...
        for (int processed_clusters = 0; processed_clusters < 4096; processed_clusters++) {
            ...
            if (!m_topk.would_enter(std::get<1>(p) * m_mu) && !m_topk.would_enter(std::get<2>(p) * m_eta)) break;
    ...
    void boundsum_timeout_query(CursorRange&& cursors_, const size_t timeout_microseconds, const float risk_factor = 1.0f)
    {
        ...
        for (int range_id = 0; range_id < cluster_sizes; range_id++) {
            float range_maxes_level_up = 0;
            float range_avgs_level_up = 0;
            auto sub_cluster_size = m_subcluster_sizes[range_id];
            for (int i = 0; i < sub_cluster_size; i++) {
        ...
        for (int processed_clusters = 0; processed_clusters < cluster_sizes; processed_clusters++) {

            auto &p = range_and_score[processed_clusters];

            if ((elapsed_latency + (risk_factor * mean_latency) > timeout_microseconds) || (!m_topk.would_enter(std::get<1>(p) * m_mu) && !m_topk.would_enter(std::get<2>(p) * m_eta))) {
                printf("clusters: %d\n", processed_clusters);
                break;
            }
            ...
            while (current_docid < end) {
                ...
                if (m_topk.insert(current_score, current_docid) && update_non_essential_lists() == UpdateResult::ShortCircuit) {
                    break;
                }
            }
            ...
            mean_latency = float(elapsed_latency) / (processed_clusters + 1);
